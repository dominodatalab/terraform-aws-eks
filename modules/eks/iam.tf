...
resource "aws_iam_role" "flyte_controlplane" {
  ...
  name  = "${local.eks_cluster_name}-flyte-controlplane"
        ...
        Condition : {
          StringEquals : {
            ...
            "${trimprefix(local.oidc_provider_url, "https://")}:sub" : [
              "system:serviceaccount:${var.flyte.platform_namespace}:flyteadmin",
              "system:serviceaccount:${var.flyte.platform_namespace}:flytepropeller",
              "system:serviceaccount:${var.flyte.platform_namespace}:datacatalog"
            ]
          }
        }
      },
...
resource "aws_iam_role_policy_attachment" "flyte_dataplane" {
  count      = var.flyte.enabled ? 1 : 0
  name       = aws_iam_role.flyte_dataplane.name
