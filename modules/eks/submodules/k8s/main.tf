...
locals {
  ...
  k8s_functions_sh = {
    ...
    content = templatefile("${local.templates_dir}/${local.k8s_functions_sh_template}", {
      ...
      ssh_extra_args                        = var.bastion_info != null ? templatestring(var.ssh_extra_args, {bastion_instance_id = var.bastion_info.instance_id}) : ""
      ...
      calico_prometheus_metrics             = var.eks_info.calico.prometheus_metrics
