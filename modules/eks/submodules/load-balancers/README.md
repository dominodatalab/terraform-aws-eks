...
| <a name="input_load_balancers"></a> [load\_balancers](#input\_load\_balancers) | List of Load Balancers to create.<br/>    [{<br/>      name     = Name of the Load Balancer.<br/>      type     = Type of Load Balancer (e.g., "application", "network").<br/>      internal = (Optional) Whether the Load Balancer is internal. Defaults to true.<br/>      ddos\_protection = (Optional) Whether to enable AWS Shield Standard (DDoS protection). Defaults to true.<br/>      listeners = List of listeners for the Load Balancer.<br/>      [{<br/>        port       = Listener port (e.g., 80, 443).<br/>        protocol   = Protocol used by the listener (e.g., "HTTP", "HTTPS").<br/>        ssl\_policy = (Optional) SSL policy to use for HTTPS listeners.<br/>        cert\_arn   = (Optional) ARN of the SSL certificate.<br/>      }]<br/>    }] | <pre>list(object({<br/>    name            = string<br/>    type            = string<br/>    internal        = optional(bool, true)<br/>    ddos_protection = optional(bool, true)<br/>    listeners = list(object({<br/>      port       = number<br/>      protocol   = string<br/>      ssl_policy = optional(string)<br/>      cert_arn   = optional(string)<br/>    }))<br/>  }))</pre> | n/a | yes |
