output "info" {
  description = <<EOF
   ...
   glue_catalog_status_table_name = "Name of the Glue Catalog table which is populated with CUR data's status."
 ...
 EOF
  value = {
    ...
    glue_catalog_table_name        = replace(local.cur_report_table_name, "-", "_")
    glue_catalog_status_table_name = aws_glue_catalog_table.aws_cur_report_status_table.name
