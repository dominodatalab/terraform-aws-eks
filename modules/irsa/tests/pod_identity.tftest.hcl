...
# and see nothing when the body is templatefile(), which is how filetask-objectstore reaches
# aws_iam_policy.filetask_objectstore. Nothing else in CI looks inside those documents.
#
...
run "filetask_objectstore_scoping" {
  ...
  assert {
    condition     = toset(jsondecode(aws_iam_policy.filetask_objectstore[0].policy).Statement[0].Condition.StringLike["s3:prefix"]) == toset(["datasets", "datasets/*"])
  ...
  assert {
    ...
    condition     = !contains(keys(jsondecode(aws_iam_policy.filetask_objectstore[0].policy).Statement[1]), "Condition")
  ...
  assert {
    condition     = jsondecode(aws_iam_policy.filetask_objectstore[0].policy).Statement[6].Resource == "arn:aws:s3:::prefixed-bucket/datasets/*"
    error_message = "The prefixed bucket's object statement must be confined to its prefix path, not the whole bucket."
  }

  assert {
    condition     = jsondecode(aws_iam_policy.filetask_objectstore[0].policy).Statement[7].Resource == "arn:aws:s3:::unprefixed-bucket/*"
  ...
  assert {
    condition     = jsondecode(aws_iam_policy.filetask_objectstore[0].policy).Statement[8].Resource == "arn:aws:s3:::kms-bucket/kms-data/*"
  ...
  assert {
    ...
    condition     = jsondecode(aws_iam_policy.filetask_objectstore[0].policy).Statement[9].Condition.StringEquals["kms:ViaService"] == "s3.eu-west-1.amazonaws.com"
...
run "filetask_objectstore_disabled" {
  ...
  assert {
    condition     = length(aws_iam_policy.filetask_objectstore) == 0
    error_message = "aws_iam_policy.filetask_objectstore must not be created when disabled."
  }

  assert {
    condition     = length(aws_iam_role.filetask_objectstore) == 0
    error_message = "aws_iam_role.filetask_objectstore must not be created when disabled."
  }

  assert {
    condition     = length(aws_iam_role_policy_attachment.filetask_objectstore) == 0
    error_message = "aws_iam_role_policy_attachment.filetask_objectstore must not be created when disabled."
  }

  assert {
    condition     = length(aws_eks_pod_identity_association.filetask_objectstore) == 0
    error_message = "aws_eks_pod_identity_association.filetask_objectstore must not be created when disabled."
  }
}

run "filetask_objectstore_rejects_name_collision" {
  command = plan
  ...
  variables {
    filetask_objectstore = {
      ...
      namespace           = "domino-compute"
      serviceaccount_name = "domino-filetask-objectstore"
      buckets             = [{ name = "collision-bucket" }]
    }
    additional_irsa_configs = [
      {
        name                = "filetask-objectstore"
        namespace           = "domino-platform"
        serviceaccount_name = "different-sa"
        policy              = jsonencode({ Version = "2012-10-17", Statement = [] })
      },
    ]
  }
  ...
  expect_failures = [
    aws_iam_role.filetask_objectstore,
  ]
}

run "filetask_objectstore_rejects_serviceaccount_collision" {
  command = plan
