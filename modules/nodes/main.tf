...
locals {
  ...
  node_group_status = {
    ...
    name => {
      ...
      # The default "platform"/"platform_jobs" node groups are excluded on purpose:
      # arm64 isn't rolled out for the platform pool
      is_arm64 = !contains(["platform", "platform_jobs"], name) && (
        try(ng.architecture, null) == "arm64" || anytrue([
